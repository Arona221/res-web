...
if (isset($_SESSION["admin"])) {
  ...
  $date1 = date("Y-m-d");
  //
  $p1_request = getConnection()->prepare("SELECT count(*) as nbr_petit_dej  FROM detail_transaction_resto WHERE resto=:resto AND type=:type and date=:date");
  $p1_request->execute(array("resto" => $nom, "type" => "peti_dej", "date" => $date));
  $r1_request = $p1_request->fetchAll(PDO::FETCH_ASSOC);
  $nbr_petit_dej = $r1_request[0]["nbr_petit_dej"];
  //
  $p1_request = getConnection()->prepare("SELECT count(*) as nbr_repas  FROM detail_transaction_resto WHERE resto=:resto AND type=:type and date=:date");
  $p1_request->execute(array("resto" => $nom, "type" => "repas", "date" => $date));
  $r1_request = $p1_request->fetchAll(PDO::FETCH_ASSOC);
  $nbr_repas = $r1_request[0]["nbr_repas"];
  //
  $p1_request = getConnection()->prepare("SELECT count(*) as nbr_diner  FROM detail_transaction_resto WHERE resto=:resto AND type=:type and date=:date");
  $p1_request->execute(array("resto" => $nom, "type" => "diner", "date" => $date));
  $r1_request = $p1_request->fetchAll(PDO::FETCH_ASSOC);
